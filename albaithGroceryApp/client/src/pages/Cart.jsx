...
const Cart = () => {
    const { products, currency, cartItems, getCartItemCount, getCartTotalAmount, updateCartItem, navigate, removeFromCart, axios, user, setCartItems } = useAppContext();
    ...
    const getCart = () => {
        ...
        for (const key in cartItems) {
            const productes = products.find((item) => item._id === key);
            productes.quantity = cartItems[key];
            tempArray.push(productes);
        }
        ...
        <div className="flex flex-col md:flex-row mt-16">
            <div className='flex-1 max-w-4xl'>
                ...
                {cartArray.map((products, index) => (
                    <div key={index} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
                        <div className="flex items-center md:gap-6 gap-3">
                            <div onClick={() => {
                                navigate(`/products/${products.category.toLowerCase()}/${products._id}`); scrollTo(0, 0)
                            }} className="cursor-pointer w-24 h-24 flex items-center justify-center border border-gray-300 rounded overflow-hidden">
                                <img className="max-w-full h-full object-cover" src={products.images[0]} alt={products.name} />
                            </div>
                            <div>
                                ...
                                <div className="font-normal text-gray-500/70">
                                    ...
                                    <div className='flex items-center'>
                                        <p>Qty:</p>
                                        <select onChange={e => updateCartItem(products._id, Number(e.target.value))} value={cartItems[products._id]} className='outline-none'>
                ...
                <button onClick={() => { navigate("/products"); scrollTo(0, 0) }} className="group cursor-pointer flex items-center mt-8 gap-2 text-primary font-medium">
                    <img src={assets.right_arrow_icon} alt="arrow" className='w-5 group-hover:-translate-x-1 transition' />
                    Continue Shopping
                </button>
            ...
            <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
                ...
                <button onClick={placeOrder} className="w-full py-3 mt-6 cursor-pointer bg-primary text-white font-medium hover:bg-primary-dull transition">
                    {paymentMethod === "COD" ? "Place Order" : "Proded to checkout"}
                </button>
            </div>
        </div>
    ) : null
