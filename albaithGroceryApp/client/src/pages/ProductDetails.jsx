...
const ProductDetails = () => {
    ...
    useEffect(() => {
        if (product) {
            const rel = (products || []).filter((p) => p.category === product.category && p._id !== product._id);
            setRelatedProducts(rel.slice(0, 5));
            setThumbnail(product.images && product.images.length ? product.images[0] : null);
        }
    }, [product, products])
    ...
    const categoryPath = product.category ? String(product.category).toLowerCase() : '';
    return (
        <div className="mt-12">
            <p>
                <Link to={"/"}>Home</Link> /
                ...
                {categoryPath && (
                    <Link to={`/products/${categoryPath}`}> {product.category}</Link>
                )} /
                ...
                <div className="text-sm w-full md:w-1/2">
                    ...
                    <div className="flex items-center gap-0.5 mt-1">
                        {Array(5).fill('').map((_, i) => (
                            <img key={i} src={assets.star_icon} className="md:w-4 w-3.5" />
                        ))}
