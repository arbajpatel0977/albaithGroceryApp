                    ...
                    </p>
                    {
                        order.items.map((item, index) => (
                            ...
                            relative bg-white text-gray-500/70 ${order.items.length !== index + 1 && "border-b"} border-gray-300 flex flex-col md:flex-row md:items-center justify-between p-4 py-5 md:gap-16 w-full max-w-4xl`}>
                                <div className='flex items-center mb-4 md:mb-0'>
                                    <div className='bg-primary/10 p-4 rounded-lg '>
                                        {item.product && item.product.images && item.product.images[0] ? (
                                            ...
                                            <div className='w-16 h-16 bg-gray-100 flex items-center justify-center text-xs text-gray-400'>Removed</div>
                                        )}
                                    </div>
                                    <div className='ml-4'>
                                        <h2 className='text-xl font-medium text-gray-800'>{item.product ? item.product.name : 'Product removed'}</h2>
                                        <p>Category : {item.product ? item.product.category : '-'}</p>
                                    </div>
                                </div>
                                ...
                                <p className='text-primary text-lg font-medium '>Amount : {currency}{item.product ? (item.product.offerPrice * item.quantity) : '0.00'}</p>
